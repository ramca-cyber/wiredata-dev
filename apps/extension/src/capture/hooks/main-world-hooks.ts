...
const WIREDATA_FETCH_SENTINEL = '__WIREDATA_FETCH_WRAPPER__';
const WIREDATA_XHR_SENTINEL = '__WIREDATA_XHR_WRAPPER__';
...
declare global {
  interface Window {
    __WIREDATA_HOOK_STATE__?: HookState;
  }
}
...
export function installPageCaptureHook(sessionId: string, tabId: number): void {
  ...
  (wrappedFetch as any)[WIREDATA_FETCH_SENTINEL] = true;
  ...
  (XMLHttpRequest.prototype.open as any)[WIREDATA_XHR_SENTINEL] = true;
    ...
    if (data) {
      ...
      this.addEventListener('load', function (this: XMLHttpRequest) {
        try {
          ...
          if (isJsonMime(contentType) && this.responseText) {
  ...
  (XMLHttpRequest.prototype.send as any)[WIREDATA_XHR_SENTINEL] = true;
}
...
export function uninstallPageCaptureHook(): void {
  ...
  if (window.fetch && (window.fetch as any)[WIREDATA_FETCH_SENTINEL] === true) {
